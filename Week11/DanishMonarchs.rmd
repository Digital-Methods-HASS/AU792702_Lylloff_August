...
List what is the separator:csv is seperated by comma"  
...
For the "csv" functions there is one column. For "csv2" functions there are 7.

4. Show the dataset so that we can see how R interprets each column

You can use glimpse() to view each column seperated and the columns datatype 

```{r eval = TRUE}

library(tidyverse)
kings <-read_csv("data/DAM - kongerækken ny 1.csv")
...
kings <- kings %>% 
  filter(!is.na(`Reign Start`)) %>% 
  filter(!is.na(`Reign End`)) %>% 
...
#By using count(), it shows that 26 monarchs had a longer than average duration of reign
...
top3 <- kings %>% 
  ...
  slice(1:3) %>% 
  mutate(Days = as.numeric(`Reign End` - `Reign Start`))
...
kings %>% 
  mutate(
    reign_duration = `Reign End` - `Reign Start`,
    year_mid = `Reign Start` + (`Reign End` - `Reign Start`) / 2
  ) %>% 
  ggplot(aes(year_mid, reign_duration)) +
  geom_point() +
  geom_smooth(colour = "red") +
  labs(
    title = "Danish monarchs duration of reign in years",
    x = "Reign - median year",
    y = "Duration of reign"
  ) +
  theme_bw() +
