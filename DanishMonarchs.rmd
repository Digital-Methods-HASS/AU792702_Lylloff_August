...
List what is the separator:_______ ".csv is seperated by comma"  
...
library(tidyverse)
kings1 <-("data/DAM - kongerækken ny.csv")

kings2 <-("data/DAM - kongerækken ny.csv")

kings3 <-("data/DAM - kongerækken ny.csv")

kings4 <-("data/DAM - kongerækken ny.csv")
...
the ones with underscore "-" and not period. For that reason, the read_csv2() function is the better option 
...
For the "csv" functions there is one colum. For "csv2" functions there are 7.

4. Show the dataset so that we can see how R interprets each column

You can use glimpse() to view each colum seperated and the colums datatype 

```{r eval = TRUE}
# COMPLETE THE BLANKS BELOW WITH YOUR CODE, then turn the 'eval' flag in this chunk to TRUE.
library(tidyverse)
kings <-read_csv("data/DAM - kongerækken ny.csv")
...
kings <- kings %>% 
  filter(!is.na(`Reign Start`), !is.na(`Reign End`)) 
kings <- kings %>% 
  mutate(
    StartReign=as.Date(paste0(`Reign Start`, "-01-01")),
    EndReign=as.Date(paste0(`Reign End`, "-01-01")),
    duration=as.numeric(EndReign - StartReign) / 365
  )
...
kings$duration
mean(as.numeric(kings$duration), na.rm = TRUE)


 
kings %>% select(duration)
kings %>% 
...
```
23 Monarchs reigned a aboved average duration 
...
top3 <- kings %>% 
  ...
  slice(1:3) %>% 
  mutate(Days = as.numeric(EndReign - StartReign))
...
kings <- kings %>% 
  mutate(midyear = `Reign Start` + (`Reign End` - `Reign Start`) / 2)
