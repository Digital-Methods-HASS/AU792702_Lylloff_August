...
```{r eval = TRUE}
# YOUR CODE HERE:
read.csv("data/DAM - kongerækken ny.csv")
kings <- read.csv("data/DAM - kongerækken ny.csv", na = c("NULL",""))

kings %>% 
  mutate(duration = Reign.End - Reign.Start) %>% 
  mutate(Midyear = Reign.End - duration/2) %>% 
  ggplot(aes(x = Midyear, y = duration))+
  geom_point( color = "blue")+
  geom_smooth()


# Like the homicides above, the trend of a king getting killed has fallen down. Kings reign longer and homicides varies alot. 
...
r markdown:
*last updated:* ´r format(sys.time(),"20-March-2026)´
